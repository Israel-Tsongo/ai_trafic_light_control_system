...
def main(sources):
	...
	print("Path: "+str(pathlib.Path.cwd().parents[0])+"/models/yolov5s.onnx")
	print("====================")
	print("vs: ",vs)
	print("vs2: ",vs2)
	print("vs3: ",vs3)
	print("vs4: ",vs4)	
	print("==========END==========")
	print("loaded model : ",str(pathlib.Path.cwd().parents[0])+"/models/yolov5s.onnx")
	
	net = cv2.dnn.readNet(str(pathlib.Path.cwd().parents[0])+"/models/yolov5s.onnx")
	
	#net = cv2.dnn.readNetFromONNX(str(pathlib.Path.cwd().parents[0])+"/models/yolov5s.onnx")
	
	print("net After loading model :",net)
...
if __name__=="__main__":
        ...
        sources =sources.split(",") # Returns list  of video path
        print(type(sources))
        main(sources)		        
